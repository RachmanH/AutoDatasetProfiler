...
const TYPE_BADGE: Record<string, string> = {
  ...
  id_like: 'bg-slate-100 text-slate-500',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <span className="self-center text-sm text-slate-400">{filtered.length} kolom</span>
      </div>

      {/* Cards */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((p) => (
          <div key={p.column} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-2 mb-3">
              <h4 className="font-semibold text-slate-800 break-all">{p.column}</h4>
              <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${TYPE_BADGE[p.detected_type] ?? 'bg-slate-100 text-slate-600'}`}>
                {p.detected_type}
              </span>
            </div>
            ...
            <div className="space-y-1 text-sm text-slate-600">
              ...
              <div className="flex justify-between">
                <span>Missing</span>
                <span className={`font-medium ${p.missing_percentage > 20 ? 'text-red-500' : ''}`}>
            ...
            {p.stats && (
              <div className="mt-3 rounded-xl bg-slate-50 p-3 text-xs text-slate-500 space-y-1">
            ...
            {p.sample_values.length > 0 && (
              <div className="mt-3">
                <p className="text-xs text-slate-400 mb-1">Contoh nilai:</p>
                <div className="flex flex-wrap gap-1">
                  {p.sample_values.slice(0, 3).map((v, i) => (
                    <span key={i} className="rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-600 truncate max-w-[100px]">
                      {String(v)}
                    </span>
                  ))}
                </div>
